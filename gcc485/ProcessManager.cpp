...
std::string ProcessManager::stop(std:: string name)
{
  ...
  if(it != _processes.end()) {
    ...
    if(pid > 0) {
      ...
      if(kill(pid, SIGTERM)) {
        LOG_ERROR_MSG("Failed to send SIGTERM to worker process:[" + ss_pid.str() + "].");
        msg = S_ERROR;
      } else {
        it->second.set_pid(0);
        LOG_INFO_MSG("Send SIGTERM to worker process:[" + ss_pid.str() + "] successful.");
        msg = S_OK;
      }
      bool needKill = true;
      for(int i = 0; i < 3; i++) {
      ...
      if(needKill) {
        if(kill(pid, SIGKILL)) {
          LOG_ERROR_MSG("Failed to send SIGKILL to worker process:[" + ss_pid.str() + "].");
          msg = S_ERROR;
        } else {
          it->second.set_pid(0);
          LOG_INFO_MSG("Send SIGKILL to worker process:[" + ss_pid.str() + "] successful.");
          msg = S_OK;
        }
      }
    } else {
...
void ProcessManager::start_worker_processes()
{
  ...
  if (pid > 0) {
    ...
    if(WIFEXITED(status)) {
      // 进程正常退出
      ss_ret << WEXITSTATUS(status);
      LOG_INFO_MSG("Worker process with PID:[" + ss_pid.str() + "] returned code:[" + ss_ret.str() + "].");
      should_relaunch = true;
    }
    if(WIFSIGNALED(status)) {
      // 被信号中断退出
      if(WCOREDUMP(status)) {
        LOG_ERROR_MSG("Worker process with PID:[" + ss_pid.str() + "] cause core dump.");
      } else {
        ss_ret << WTERMSIG(status);
        LOG_WARN_MSG("Worker process with PID:[" + ss_pid.str() + "] terminated by signal:[" + ss_ret.str() + "].");
      }
      should_relaunch = true;
    }
    if(WIFSTOPPED(status)) {
      // 被信号中断停止
      ss_ret << WSTOPSIG(status);
      LOG_INFO_MSG("Worker process with PID:[" + ss_pid.str() + "] stopped by signal:[" + ss_ret.str() + "].");
    }
    if(WIFCONTINUED(status)) {
      // 收到SIGCONT信号，继续运行
      LOG_INFO_MSG("Worker process with PID:[" + ss_pid.str() + "] continued.");
    }
    if(should_relaunch) {
      relaunch(pid);
    }
    ...
    for(std::map<std::string, WorkerProcess>::iterator it = _processes.begin(); it != _processes.end(); it++) {
      if(!_config_file->has_section(it->first)) {
        ...
        if(pid > 0) {
          std::stringstream ss_pid;
          ss_pid << pid;
          LOG_INFO_MSG("Worker process:[" + it->first + "] are still running with PID:[" + ss_pid.str() + "].");
        } else {
